...
class Layer1TemporalConstruction:
    ...
    def process_chunk(self, windows_list: List[Dict]) -> pd.DataFrame:
        ...
        if len(df):
            ...
            print(f"{n_fallback}/{len(df)} windows have insufficient response-time coverage "
            ...
            if 'n_correct_obs' in df.columns:
                ...
                if n_no_correct:
                    print(f"{n_no_correct}/{len(df)} windows have NO IsCorrect observations and carry NaN for success_rate/success_trend.")
        return df
...
class Layer2StructureLearning:
    ...
    def __init__(self, max_lag: int = 2, alpha: float = 0.1, pc_alpha: float = 0.2,
                 corr_threshold: float = 0.97, min_effect: float = 0.12):
    ...
    def attach_experimental_keys(self, layer1_df: pd.DataFrame, mapping_df: pd.DataFrame,
                                  construct_col_in_layer1: str = 'dominant_construct',
                                  construct_col_in_mapping: str = 'ConstructId',
                                  validate_year_consistency: bool = False) -> pd.DataFrame:
        ...
        if len(ambiguous):
            print(f"WARNING: {len(ambiguous)} ConstructId value(s) map to more than one "
                  f"(Treatment, Question, Year) row in mapping_df, e.g. {ambiguous.index.tolist()[:5]}.")

        keyed = layer1_df.merge(
            mapping_df[list(required_mapping)],
        ...
        print(f"attach_experimental_keys: {len(layer1_df)} Layer 1 windows -> {len(keyed)} keyed rows "
        ...
        if validate_year_consistency:
            if 'year' not in keyed.columns:
                raise ValueError("validate_year_consistency=True but 'year' (derived) column is missing.")
            mismatch = keyed['year'] != keyed['Year']
            if mismatch.any():
                n_bad = int(mismatch.sum())
                raise ValueError(f"{n_bad}/{len(keyed)} rows have derived window year != mapped Year.")

        self._require_experimental_keys(keyed)
        return keyed.reset_index(drop=True)

    def _regularize_time_index(self, agg: pd.DataFrame, bucket_freq: str = 'D',
                                max_gap_to_interpolate: int = 2) -> Tuple[pd.DataFrame, Dict]:
        ...
        if n_missing_total:
            print(f"  time regularization: {n_before} observed buckets -> {len(reg)} regular {bucket_freq}-buckets "
                  f"({n_missing_total} gap bucket(s), {n_interpolated} interpolated, {n_unrecoverable} left as NaN)")
        return reg, stats

    def _fill_remaining_gaps(self, agg: pd.DataFrame, feature_cols: List[str]) -> Tuple[pd.DataFrame, List[str], Dict[str, int]]:
        unfillable_cols = []
        n_filled = {}
        for col in feature_cols:
    ...
    def aggregate_time_series_by_experimental_key(
        self, keyed_layer1_df: pd.DataFrame,
        bucket_freq: str = 'D',
        ...
        print(f"{n_ok}/{len(key_status)} (Treatment, Question, Year) triples "
              f"have >= {min_obs_per_entity} regular {bucket_freq}-buckets with no unfillable columns and got a temporal model")
    ...
    def check_experimental_key_sparsity(self, keyed_layer1_df: pd.DataFrame, bucket_freq: str = 'D',
                                         min_obs: int = 30) -> pd.Series:
        self._require_experimental_keys(keyed_layer1_df)
        df = keyed_layer1_df.copy()
        df['time_bucket'] = df['window_start_time'].dt.floor(bucket_freq)
        counts = df.groupby(
            ['TreatmentLessonConstructId', 'QuestionConstructId', 'Year']
        )['time_bucket'].nunique().sort_values(ascending=False)
        n_ok = (counts >= min_obs).sum()
        print(counts.describe())
        print(f"{n_ok}/{len(counts)} (Treatment, Question, Year) triples have >= {min_obs} {bucket_freq}-buckets "
              f"(observed buckets only, not the regularized elapsed-period count)")
        return counts

    def prepare_timeseries_matrix(self, agg_df: pd.DataFrame,
                                   feature_contract: Optional[List[str]] = None) -> Tuple[np.ndarray, pd.DataFrame]:
        ...
        for i in range(len(cols)):
            for j in range(i + 1, len(cols)):
                ...
                if corr_matrix.loc[col_i, col_j] > self.corr_threshold:
                    print(f"Dropping '{col_j}': correlation with '{col_i}' is "
    ...
    def run_pcmci_stability_selection(self, feature_df: pd.DataFrame, n_bootstrap: int = 100,
                                       block_size: int = 5, stability_threshold: float = 0.8,
        ...
        print(f"stability selection: {successful_runs}/{n_bootstrap} bootstrap runs succeeded, "
    ...
    def run_lag_sensitivity(self, feature_df: pd.DataFrame,
                             lag_candidates: Optional[List[int]] = None) -> Dict[int, Dict]:
    ...
    def run_pcmci_per_entity(self, per_entity_agg: Dict[Tuple, pd.DataFrame],
    ...
    def build_layer2(self, keyed_layer1_df: pd.DataFrame,
                     held_out_user_ids: Optional[Iterable] = None,
                     bucket_freq: str = 'D',
                     min_obs_per_entity: int = 30,
                     max_gap_to_interpolate: int = 2,
        ...
        causal_graphs, stability_results = self.run_pcmci_per_entity(
            per_entity_agg, feature_contract=feature_contract,
    ...
    def predict_for_key(self, causal_graphs: Dict[Tuple, Dict],
                        treatment_id: Any, question_id: Any, year: int) -> Optional[Dict]:
        key = (treatment_id, question_id, year)
        return causal_graphs.get(key)


def run_layer1_pipeline(raw_logs: pd.DataFrame, window_size: int = 50, step_size: int = 50,
                        min_dominant_share: Optional[float] = None) -> pd.DataFrame:
...
def run_full_pipeline(raw_logs: pd.DataFrame,
    ...
    layer2 = Layer2StructureLearning(
        max_lag=max_lag, alpha=alpha, pc_alpha=pc_alpha,
        corr_threshold=corr_threshold, min_effect=min_effect
    )
...
def run_layer2_pipeline(keyed_layer1_df: pd.DataFrame,
                        max_lag: int = 2,
                        alpha: float = 0.1,
                        pc_alpha: float = 0.2,
                        corr_threshold: float = 0.97,
